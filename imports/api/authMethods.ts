...
const createDefaultTasks = async (userId: string) => {
  ...
  if (
    user?.profile &&
    "defaultTasksCreated" in user.profile &&
    user.profile.defaultTasksCreated
  ) {
    return;
  }
...
Meteor.methods({
  async "auth.register"({ username, password }) {
    ...
    const existingUser = await UsersCredentialsCollection.findOneAsync({
      username
    });
    if (existingUser) {
  ...
  async "auth.login"({ username, password }) {
    ...
    if (!meteorUser) {
      ...
      throw new Meteor.Error(
        "user-deleted",
        "Ce compte utilisateur a été supprimé"
      );
    }
  ...
  async "auth.logout"() {
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "Vous devez être connecté pour vous déconnecter"
      );
    }
  ...
  "users.search": function ({ username }: { username: string }) {
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "Vous devez être connecté pour rechercher des utilisateurs"
      );
    }

    if (!username || typeof username !== "string") {
      throw new Meteor.Error("invalid-data", "Nom d'utilisateur invalide");
    }
