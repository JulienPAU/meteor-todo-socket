...
export const UsersList = ({ currentUserId, selectedUserId, onSelectUser }: UsersListProps) => {
    ...
    return (
        <div className="users-list">
            <h2>Conversations</h2>
            {users.length === 0 ? (
                <div className="empty-users-message">Aucun utilisateur disponible pour le moment</div>
            ) : (
                users.map((user: User) => (
                    <div key={user._id} className={`user-item ${selectedUserId === user._id ? "active" : ""} ${unreadCounts[user._id] > 0 ? "unread" : ""}`} onClick={() => onSelectUser(user._id, getDisplayName(user))}>
                        <div className="user-avatar">{getInitial(user)}</div>
                        <div className="user-info">
                            <div className="user-name">
                                {getDisplayName(user)}
                                {isUserOnline(user) && <span className="online-indicator" title="En ligne"></span>}
                            </div>
                        </div>
                        {unreadCounts[user._id] > 0 && (
                            <div className="unread-count" title={`${unreadCounts[user._id]} message(s) non lu(s)`}>
