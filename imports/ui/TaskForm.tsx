...
import {
  ...
  UserActivity
} from "../api/UserActivityCollection";
...
export const TaskForm = ({ groupId }: TaskFormProps) => {
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newText.trim() && currentUser) {
      if (groupId) {
        Meteor.call(
          "userActivity.setTyping",
          currentUser.username || "Quelqu'un",
          "typing",
          undefined,
          groupId
        );
      ...
      return (
        <em>
          <span
            style={{
              color:
                groupId && otherUserTyping[0].color
                  ? otherUserTyping[0].color
                  : undefined
            }}
          >
    ...
    <div className="task-form-container">
      <form className="task-form" onSubmit={handleSubmit}>
        ...
        <input
          ...
          placeholder={
            groupId
              ? "Ajouter une nouvelle tâche au groupe"
              : "Saisissez une nouvelle tâche personnelle"
          }
          value={text}
      ...
      {otherUserTyping.length > 0 && (
        <div
          ...
          style={{
            borderLeft:
              groupId && otherUserTyping[0].color
                ? `3px solid ${otherUserTyping[0].color}`
                : undefined
          }}
        >
          {getTypingMessage()}
